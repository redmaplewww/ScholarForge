...
// Embedded API credentials (hardcoded, compiled into binary)
// eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
process.env.ANTHROPIC_API_KEY = 'sk-cp-OwJtuF3iziNYnPHLv17eAbjzTtvUN6-hkk8EqmREjgg4hzWNjNN-lw0uG-EsphovUVF9HyXYSgjCCccuF8yg1Z982QfZvEOyTybDGgOHvpmzESiFHuZraPY'
// eslint-disable-next-line custom-rules/no-top-level-side-effects, custom-rules/no-process-env-top-level
...
async function main(): Promise<void> {
  ...
  if (process.argv[2] === '--angsheng-in-chrome-mcp') {
    ...
    const { runAngshengInChromeMcpServer } = await import(
    ...
    const { runChromeNativeHost } = await import(
      '../utils/angshengInChrome/chromeNativeHost.js'
    )
  ...
  if (
    ...
    console.error(
      `[deprecated] Use: aura daemon ${mapped}${args[1] ? ' ' + args[1] : ''}`,
    )
    profileCheckpoint('cli_daemon_path')
    const { enableConfigs } = await import('../utils/config.js')
    enableConfigs()
  ...
  if (
    ...
    console.error(
      `[deprecated] Use: aura job ${args[0]} ${args.slice(1).join(' ')}`.trim(),
    )
    profileCheckpoint('cli_templates_path')
    const { templatesMain } = await import('../cli/handlers/templateJobs.js')
