...
> 实现状态：WorkflowTool / DSL scripts 仍以 Stub / 部分实现为主；LAMMPS WorkflowEngine 是另一套已接入的阶段式 workflow
> 引用数：10

## 当前 workflow 分层

当前仓库中有三类容易混淆的 workflow 能力：

| 层级 | 入口 | 状态 | 说明 |
|------|------|------|------|
| Workflow style | `/workflow guided`、`/workflow auto`、`/workflow escalate` | 已实现 | 改变主循环的协作风格和系统提示，不等于多步骤 DSL 执行器 |
| LAMMPS WorkflowEngine | `/workflow-guided <task>`、`/workflow-semi <task>`、`/workflow-auto <task>` | 已接入 | 面向 LAMMPS 的硬编码阶段流程，包含 WF-00 到 WF-05、stage prompt、idle 续推 |
| WorkflowTool / DSL scripts | `packages/builtin-tools/src/tools/WorkflowTool/*`、`.angsheng/workflows/` | Stub / 部分实现 | 目标是文件式 YAML/JSON 多 agent 工作流，但执行器和 UI 尚未完成 |

本文主要描述第三层：`WORKFLOW_SCRIPTS` feature 下的文件式 workflow DSL。LAMMPS 阶段式 workflow 见 `docs/features/lammps-workflow-engine.md`。

## 一、功能概述

WORKFLOW_SCRIPTS 的目标是实现基于文件的多步自动化工作流。用户可以定义 YAML/JSON 格式的工作流描述文件，系统将其解析为可执行的多 agent 步骤序列。提供 `/workflows` 命令管理和触发工作流。

注意：这不是当前 LAMMPS `/workflow-auto` 使用的执行路径。LAMMPS 当前走 `src/utils/workflowEngine.ts`，而不是 `LocalWorkflowTask`。
